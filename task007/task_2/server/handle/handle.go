...
import (
	...
	"github.com/karamel29/go-courses/task007/task_2/server/handle/methods"
)
...
func parseRequest(r *http.Request) (*Request, error) {
	...
	if err = json.Unmarshal(bodyData, req); err != nil {
		log.Print(err.Error())
		return nil, errors.New("incorrect request format")
	}
	return req, nil
}

func handle(req Request) (interface{}, error) {
	handlers := map[string]func(message json.RawMessage, id int) (interface{}, error){
		"register":    methods.Register,
		"list":       methods.List,
	}
	h, ok := handlers[req.Method]
	if !ok {
		return nil, errors.New(fmt.Sprintf("not supported method %s", req.Method))
	}
	resp, err := h(req.Params, int(req.Id))
	if err != nil {
		return nil, err
	}
	return resp, nil
